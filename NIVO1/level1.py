...
# Load pizza stages and background
table_img = pygame.image.load("background.png")
...
# Pizza area (just for visual, no drop) - прикажи на дното центрирано
pizza_area_pos = (WIDTH // 2 - pizza_size[0] // 2, HEIGHT - pizza_size[1] - int(HEIGHT*0.05))

# List to store placed ingredients info (name and rect where to draw in brown square)
placed_positions = [None] * len(correct_order)

# Копчиња Restart и Следно (3D стил)
...
next_button_rect = pygame.Rect(WIDTH - button_width - 20, 20 + button_height + button_padding, button_width, button_height)

def draw_3d_button(rect, text, base_color, light_color, font, screen):
    # Полн правоаголник
    pygame.draw.rect(screen, base_color, rect, border_radius=10)
    # Светла ивица (горе)
    pygame.draw.line(screen, light_color, (rect.left+5, rect.top+5), (rect.right-5, rect.top+5), 3)
    pygame.draw.line(screen, light_color, (rect.left+5, rect.top+5), (rect.left+5, rect.bottom-5), 3)
    # Текст центриран
...
def reset_game():
    global current_index, ingredients, placed_positions, shuffled_names
...
def draw_screen():
    ...
    # Draw buttons
    draw_3d_button(restart_button_rect, "Restart", LIGHT_BROWN, WHITE, font, screen)
    draw_3d_button(next_button_rect, "Следно", BROWN, LIGHT_BROWN, font, screen)
...
while running:
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            # Проверка дали кликнал на Следно копче
