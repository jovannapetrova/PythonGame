...
SHAPE_COLORS = {
    'круг': (255, 100, 100),
    'триаголник': (255, 215, 0),
    'квадрат': (100, 200, 255)
}

all_shapes = ['круг', 'триаголник', 'квадрат']
shape_size = 30

# Shape drawing functions
def draw_circle(x, y, size=shape_size):
    pygame.draw.circle(screen, SHAPE_COLORS['круг'], (x, y), size)

def draw_square(x, y, size=shape_size):
    pygame.draw.rect(screen, SHAPE_COLORS['квадрат'], (x - size, y - size, size * 2, size * 2), border_radius=8)

def draw_triangle(x, y, size=shape_size):
    points = [(x, y - size), (x - size, y + size), (x + size, y + size)]
    pygame.draw.polygon(screen, SHAPE_COLORS['триаголник'], points)
...
def draw_shape_with_size(shape, x, y, size):
    if shape == 'круг':
        ...
        pygame.draw.rect(screen, SHAPE_COLORS['квадрат'], (x - size, y - size, size * 2, size * 2), border_radius=6)
    elif shape == 'триаголник':
        points = [(x, y - size), (x - size, y + size), (x + size, y + size)]
        pygame.draw.polygon(screen, SHAPE_COLORS['триаголник'], points)
